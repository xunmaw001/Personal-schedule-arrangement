<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="日程编号" prop="richengbianhao">
          <el-input v-model="ruleForm.richengbianhao" 
              placeholder="日程编号" clearable  :readonly="ro.richengbianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="日程编号" prop="richengbianhao">
              <el-input v-model="ruleForm.richengbianhao" 
                placeholder="日程编号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="倒数日" prop="daoshuri">
          <el-input v-model="ruleForm.daoshuri" 
              placeholder="倒数日" clearable  :readonly="ro.daoshuri"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="倒数日" prop="daoshuri">
              <el-input v-model="ruleForm.daoshuri" 
                placeholder="倒数日" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `gongzuorichengzhuanfa/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gongzuorichengzhuanfa/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gongzuorichengzhuanfaCrossAddOrUpdateFlag = false;
                  this.parent.search();
